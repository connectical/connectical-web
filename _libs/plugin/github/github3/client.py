...
from . import request
from .link_parser import parse_link_value
...
class Client(request.Request):
  ...
  def list_repo(self, user=None, **kw):
    """Return a PaginatedResourceList of all of the authenticated user's repos"""
    if user is None:
        url = "%s/user/repos" % (self.BASE_URL)
    else:
        self.user = user
        url = "%s/users/%s/repos" % (self.BASE_URL, user)
