...
function get_tweets(user) {
		$.ajax({
			url: "https://api.twitter.com/1/statuses/user_timeline/" + user + ".json?count=5", 
			type: 'GET',
			dataType: 'JSONP',
			success: function(data) {
				$(data).each(function (i,o) {
					var code = $("#post-tweet-tpl").clone();
					$(code).find("p").html(o.text);
					$(code).find(".entry-author").html(
						'<a href="https://twitter.com/' + o.user.screen_name + '">@' + o.user.screen_name + "</a>");
					$(code).find(".entry-date").html(new Date(o.created_at).toISOString().split("T")[0]);
	...
	$.ajax({
		...
		success: function(data) {
			...
			$.each(data.data.filter(function(o) {
				...
				var code = $("#post-ghactivity-tpl").clone();
				$(code).find(".entry-author").html(
