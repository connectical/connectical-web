...
def qa_test(project):
    if len(project.name) == 0:
        print "[build]   QA: Malformed data, project has no name!"
        return False

    if len(project.description) == 0:
        print "[build]   QA: Project %s has no description." % project.name
        return False

    if len(project.author) == 0:
        print "[build]   QA: Project %s has no author." % project.name
        return False

    if len(project.url) == 0:
        print "[build]   QA: Project %s has no URL." % project.name
        return False

    if project.updated is None:
        print "[build]   QA: Project %s has not updated time." % project.name
        return False
...
for person in Site.CONTEXT.config.staff.sections():
    ...
    try:
        ...
        try:
            ...
            print "[build]   Skip products for %s. Not supported engine: " % person, engine

    except NoOptionError:
        print "[build]   Skip products for %s. None engine defined." % person
